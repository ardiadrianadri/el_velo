...
describe('review publication helpers', () => {
    it('publishes a review with the expected GitHub request', async () => {
        ...
        expect(mocks.axiosPost).toHaveBeenCalledWith('https://api.github.com/repos/owner/repository/pulls/42/reviews', expect.objectContaining({ commit_id: 'abc123', event: 'COMMENT', comments: [] }), expect.objectContaining({ timeout: 300_000 }));
    });
    it('falls back to a summary-only review after a GitHub inline-comment validation error', async () => {
        process.env.GITHUB_REPOSITORY = 'owner/repository'; process.env.GITHUB_TOKEN = 'github-token'; process.env.PR_NUMBER = '42'; process.env.PR_HEAD_SHA = 'abc123';
        const consoleWarn = vi.spyOn(console, 'warn').mockImplementation(() => undefined);
        mocks.axiosPost.mockRejectedValueOnce({ isAxiosError: true, response: { status: 422, data: { message: 'Invalid line' } } }).mockResolvedValueOnce({});
        await writeReview({ findings: [finding()] }, diff);
        expect(mocks.axiosPost).toHaveBeenCalledTimes(2);
        expect(mocks.axiosPost).toHaveBeenNthCalledWith(2, expect.any(String), expect.objectContaining({ comments: [] }), expect.anything());
        expect(consoleWarn).toHaveBeenCalledWith(expect.stringContaining('rejected inline'), { message: 'Invalid line' });
    });
